										 ...
										 mixer_element_type type ) 
{
	...
	case CAPTURE:
		if (snd_mixer_selem_has_capture_volume( mixer_element )) {
			snd_mixer_selem_get_capture_volume_range( mixer_element, &volume_min, &volume_max );
			loge( "volume_max = %d, volume_min = %d\n", volume_max, volume_min );
...
* @param[in] bIsAD  			AD or DA.
* @return T_S32
* @retval if return 0 success, otherwise failed 
*/
 T_S32 OpenAlsa( alsa_handle_t * handle, T_pSTR strDev, T_BOOL bIsAD )
{
	T_S32 ret = 0;
...
* @retval if return 0 success, otherwise failed 
*/
 T_S32 setHardwareParams( alsa_handle_t *handle )
{
	...
	err = snd_pcm_hw_params_set_rate_near( handle->handle, hardwareParams, &requestedRate, 0 );
	if (err < 0)
   ...
   // if (err < 0) {
    ...
    err = snd_pcm_hw_params_set_buffer_time_near( handle->handle, hardwareParams, &latency, NULL );
	if (err < 0) {
...
* @retval if return 0 success, otherwise failed 
*/
 T_S32 setSoftwareParams( alsa_handle_t *handle )
{
...
* @retval if return 0 success, otherwise failed 
*/
 T_S32 SetCtrlVal( alsa_handle_t *handle, T_pCSTR name, T_U32 value, T_S32 index )
{
 ...
 T_S32 OpenAD( alsa_handle_t * handle, T_pSTR strDev )
{
	...
	//SetMixerLevel( handle, MIC_CAPTURE_VOLUME, 50);
...
{
	assert( handle );
	printf("audio_recorder->SetLineIN \n");
	if ( !(handle->bIsAD) ) {
		logw( "capture can't set line in!\n" );
		return 1;
	}
