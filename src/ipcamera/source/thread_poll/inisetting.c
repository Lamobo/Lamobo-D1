...
int IniSetting_SetPictureInfo( struct picture_info *pic )
{
	...
	ret += iniparser_set(setting.ini, "picture:osd_name", pic->osd_name);
	ret += iniparser_set(setting.ini, "picture:osd_place", pic->osd_place);
	ret += iniparser_set(setting.ini, "picture:osd_time", pic->osd_time);
	if (ret !=0) {
		print_error("%s:iniparser image param set fail!", __func__);
		return -1;
	}
