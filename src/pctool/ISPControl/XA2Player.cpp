...
HRESULT XA2Player::Initialize(const WAVEFORMATEX* format, int index, bool isMusicPlayback)
{
	if (NULL == m_xaudio2 && m_ds == NULL) {
		AfxMessageBox(L"Cannot play audio, this problem is due to DX version on your PC is too low, or the sound card driver does not support the new features of the DX!", 0, 0);
		return E_FAIL;
	}
	...
	if (m_ds) { //使用的是DirectSound
		...
		if (!hWnd) {
			AfxMessageBox(L"Cannot play audio, can not obtain coordinate window to DirectSound!", 0, 0);
			return E_FAIL;
		}

		if (!((CStreamAudio*)m_ds)->Open(hWnd, (CAudioStreamHandler*)this, format)) {
			AfxMessageBox(L"Cannot play audio, unable to open the DirectSound!", 0, 0);
			return E_FAIL;
		}
