...
::noteIncomingPacket(u_int16_t seqNum, u_int32_t rtpTimestamp,
  ...
  if (seqNumLT((u_int16_t)oldSeqNum, seqNum)) {
    ...
    if (seqNumDifference >= 0x8000) {
      ...
      seqNumCycle += 0x10000;
	}else if (seqNumDifference > 1) {
		fLostPacketCnt += seqNumDifference;
		fprintf(stderr, "%p::(seqnum old = %d, cur = %d)current lost %d frame, amount lost: %d from start!\n", this, oldSeqNum, seqNum, seqNumDifference, fLostPacketCnt);
	}
